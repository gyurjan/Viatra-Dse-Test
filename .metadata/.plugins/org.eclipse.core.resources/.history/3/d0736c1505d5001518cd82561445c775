...
public final class AllTokenEndQuerySpecification extends BaseGeneratedEMFQuerySpecification<AllTokenEndMatcher> {
  ...
  public AllTokenEndMatch newMatch(final Object... parameters) {
    return AllTokenEndMatch.newMatch((PetriNet.Place) parameters[0]);
  }
  ...
  private static class GeneratedPQuery extends BaseGeneratedEMFPQuery {
    ...
    public Set<PBody> doGetContainedBodies() throws QueryInitializationException {
      ...
      try {
      {
      	...
      	new TypeConstraint(body, new FlatTuple(var_P), new EClassTransitiveInstancesKey((EClass)getClassifierLiteral("Petrinet", "Place")));
      	new NegativePatternCall(body, new FlatTuple(var_P), PlacesWithTokensExceptEndPlacesQuerySpecification.instance().getInternalQueryRepresentation());
      	bodies.add(body);
      }
